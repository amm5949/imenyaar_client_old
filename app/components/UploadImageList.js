...
import {Dimensions, Image, Platform, StyleSheet, TouchableHighlight, TouchableOpacity, View} from "react-native";
...
const EmptyList = (setImages) => (
    <View style={styles.container}>
        ...
        <AppButton
            ...
            onPress={()=>setImages([require("../assets/list_report_screen/building(1).jpg")])}
        />
    </View>
)
...
const UploadButton = (images, setImages) => (
    <View style={styles.uploadButtonContainer}>
        <TouchableHighlight
            onPress={() => setImages([...images, require("../assets/list_report_screen/building(2).jpg")])}
            style={styles.uploadButtonView}
            underlayColor={colors.yellow + "55"}
        >
            <MaterialCommunityIcons name={"cloud-upload"} size={26} color={colors.yellow}/>
        </TouchableHighlight>
    </View>
)
...
const ImageView = (imageIndex, images, setImages) => (
    <View style={{alignItems: "center"}} >
        <Image
            ...
            resizeMode="cover"
        />
        {images[imageIndex] && <TouchableOpacity
            style={{marginTop: 5}}
            onPress={() => setImages(images.filter((value, index) => index !== imageIndex))}
        >
...
const ImageList = (images, setImages, rangeStart, setRangeStart) => {
    const setRange = (next) => {
        ...
        else if (next && rangeStart + 3 < images.length) setRangeStart(rangeStart + 3);
    ...
    return(
        <View style={styles.imageListView}>
            ...
            {rangeStart===0 ? UploadButton(images, setImages) : ImageView(rangeStart, images, setImages)}
...
function UploadImageList(props) {
    ...
    return (
        <>
            {images.length === 0 ? EmptyList(setImages) : ImageList(images, setImages, rangeStart, setRangeStart)}
        </>
    );
}
